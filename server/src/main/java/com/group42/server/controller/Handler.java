...
public class Handler {
    ...
    public static void start(InputMessage object) {
        if (object != null) {
            switch (object.getActionId()) {
                ...
                case 40:
                    lOGGER.info("case 40");
                    setStatus("leaveGroup");
                    break;
                case 41:
                    lOGGER.info("case 41");
                    setStatus("leavePrivateChat");
                    break;
                case 42:
                    lOGGER.info("case 42");
    ...
    private static void registration(InputMessage object) {
        lOGGER.debug(object.getLogin()+ " "  +object.getPassword()+ " "  +
                object.getEmail()+ " "  +object.getSecondName()+ " "  +object.getBirthday()+ " "  +object.getPhoneNumber() + " "  +object.getFirstName());
        ...
        if (msgUsr.getActionId() == 1) {
            ...
            if (usrList.size() != 0) {
                for (User user : usrList) {
                        lOGGER.debug("password from database: " + user.getPassword() + " " + "password from client " + msgUsr.getPassword());
