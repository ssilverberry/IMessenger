...
public class ServerWorker extends Thread {
    ...
    private void createPrivateChat() {
        ...
        List<com.group42.server.controller.ServerWorker> serverWorkers = server.getWorkerList();
        ...
        for (com.group42.server.controller.ServerWorker i : serverWorkers) {
    ...
    private void deleteUserFromGroup() {
        if (ChatRoomsDAOimpl.getInstance().isPrivateChatById(mesage.getChatId())) {
            ...
            for (com.group42.server.controller.ServerWorker worker : server.getWorkerList()) {
    ...
    private void messageToGroupChat() {
        List<com.group42.server.controller.ServerWorker> workerList = server.getWorkerList();
        ...
        for (ChatRoom user : chatUsersList) {
            for (com.group42.server.controller.ServerWorker worker : workerList) {
    ...
    private void addMembersToGroupChat() {
        ...
        for (com.group42.server.controller.ServerWorker serverWorker : server.getWorkerList()) {
    ...
    private void sendMessageToPrivateRoom(String fromUser, String toUser, String msgBody) {
        List<com.group42.server.controller.ServerWorker> newServerWorkerList = server.getWorkerList();
        OutMessage out = new OutMessage(fromUser, toUser, msgBody, 33);
        String line = transformOut(out);
        for (com.group42.server.controller.ServerWorker serverWorker : newServerWorkerList) {
    ...
    private void sendList() {
        ...
        List<com.group42.server.controller.ServerWorker> mylist3 = server.getWorkerList();
        for (com.group42.server.controller.ServerWorker i : mylist3) {
            i.send(line);
        }
    }
    ...
    private void sendChatHistory(Integer chatId, String fromUser) {
        ...
        for (com.group42.server.controller.ServerWorker i : server.getWorkerList()) {
